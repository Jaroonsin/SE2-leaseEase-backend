...
func SendLessorAcceptanceEmail(req *dtos.ApprovalReservationDTO) error {
	cfg := config.LoadConfig()
...
func SendLessorDeclineEmail(lesseeEmail, propertyName string) error {
	cfg := config.LoadConfig()
	...
	m.SetHeader("From", cfg.EmailUser)
	m.SetHeader("To", lesseeEmail)
	m.SetHeader("Subject", "LeaseEase - Rental Request Declined")
	...
	emailBody := fmt.Sprintf(`
		...
		<body>
			<div class="container">
				<p class="header">LeaseEase - Rental Request Declined</p>
				<p class="content">Dear Valued Lessee,</p>
				<p class="content">We regret to inform you that your request to lease <strong>%s</strong> has been declined.</p>
				<p class="content">While this particular opportunity did not work out, we encourage you to explore other available properties on our platform.</p>
				<p class="content">Should you require any assistance, please do not hesitate to contact us.</p>
				<p class="footer">Thank you for considering LeaseEase.<br>Best regards,<br><strong>The LeaseEase Team</strong></p>
			</div>
		</body>
		</html>
	`, propertyName)
